...
extension ImageResizer {
  ...
  static func resizeImageWithInstruction(imageSource: CGImageSource, instruction: ResizeInstruction) -> CGImage? {
    ...
    guard let context = CGContext(data: nil, width: scaledSize.width, height: scaledSize.height, bitsPerComponent: thumbnail.bitsPerComponent, bytesPerRow: 0, space: thumbnail.colorSpace ?? CGColorSpaceCreateDeviceRGB(), bitmapInfo: thumbnail.bitmapInfo.rawValue) else {
      print("Failed to create CGContext.")
      return nil
    }
    
    context.interpolationQuality = .high
    applyOrientationTransform(context: context, orientation: orientation, size: CGSize(width: scaledSize.width, height: scaledSize.height))
